...
{{Figure "Transactions (tx1, tx2, tx3) mapped to HTTP requests at a 1:1 ratio." (NewImgSrcAndAlt "/assets/images/http-transactions/http-transactions.svg" "Transactions (tx1, tx2, tx3) mapped to HTTP requests at a 1:1 ratio.")}}
...
{{Figure "A data race causing two concurrent HTTP requests to insert the same row." (NewImgSrcAndAlt "/assets/images/http-transactions/concurrent-race.svg" "A data race causing two concurrent HTTP requests to insert the same row.")}}
...
{{Figure "An aborted transaction being retried within the same request." (NewImgSrcAndAlt "/assets/images/http-transactions/transaction-retry.svg" "An aborted transaction being retried within the same request.")}}
