...
{{Figure "Number of jobs in queue. One hour into a long-lived transaction, we're at 60k jobs." (ImgSrcAndAlt "/assets/images/postgres-queues/pre-queue-count.png" "Number of jobs in queue. One hour into a long-lived transaction, we're at 60k jobs.")}}
...
{{Figure "Median lock time. Normally < 0.01 s, locks are taking 15x longer than that one hour in." (ImgSrcAndAlt "/assets/images/postgres-queues/pre-lock-time.png" "Median lock time. Normally < 0.01 s, locks are taking 15x longer than that one hour in.")}}
...
{{Figure "Number of dead tuples in the jobs table. The curve flattens out as jobs get harder to work." (ImgSrcAndAlt "/assets/images/postgres-queues/pre-dead-tuples.png" "Number of dead tuples in the jobs table. The curve flattens out as jobs get harder to work.")}}
...
{{Figure "Que finding a job under ideal conditions." (ImgSrcAndAlt "/assets/images/postgres-queues/index.svg" "Que finding a job under ideal conditions.")}}
...
{{Figure "Que trying to find a lock in a bloated heap." (ImgSrcAndAlt "/assets/images/postgres-queues/index-bloated.svg" "Que trying to find a lock in a bloated heap.")}}
...
{{Figure "Que finding a lock with a greater index specificity despite a bloated heap." (ImgSrcAndAlt "/assets/images/postgres-queues/index-corrected.svg" "Que finding a lock with a greater index specificity despite a bloated heap.")}}
...
{{Figure "Number of jobs in the queue with patched version of Que. 30k one hour in." (ImgSrcAndAlt "/assets/images/postgres-queues/post-queue-count.png" "Number of jobs in the queue with patched version of Que. 30k one hour in.")}}

And for comparison, here's what it looked like _before_ the patch:

{{Figure "Number of jobs in the queue on vanilla Que. 60k one hour in." (ImgSrcAndAlt "/assets/images/postgres-queues/pre-queue-count.png" "Number of jobs in the queue on vanilla Que. 60k one hour in.")}}
...
{{Figure "Job lock time with patched Que. Spikes very suddenly, which results in queue count also falling off a cliff." (ImgSrcAndAlt "/assets/images/postgres-queues/post-lock-time.png" "Job lock time with patched Que. Spikes very suddenly, which results in queue count also falling off a cliff.")}}
