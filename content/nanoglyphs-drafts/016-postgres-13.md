...
I’m covering some favorite highlights, but see the [full release notes](https://www.postgresql.org/docs/release/13.0/) because I’m not even hitting all the big ones, some of which require a little too much backstory for this newsletter. As with every Postgres release there are dozens of improvements of all sizes. Once again Postgres demonstrates by example how an open-source product can be far better than a commercial one, as almost none of these major wins would have floated to the top of a private company’s stack ranking.

### Index deduplication (#index-deduplication)

Postgres 13's headerliner is [deduplication in B-tree indexes](https://git.postgresql.org/gitweb/?p=postgresql.git;a=commit;h=0d861bbb702f8aa05c2a4e3f1650e7e8df8c8c27), which allows duplicate values in an index to share space and thereby vastly reduce index sizes in some cases.

Claiming specific numbers on space savings is a little fraught because it depends it depends so much on the characteristics of the property being indexed and on workload, but the patch suggests that a sample index where each unique value is duplicated 10-15 times might be reduced in size by 2.5 to 4x (meaning an index occupying 100 GB would become 25 GB). This is a very big deal because in the real world Postgres tables tend to have a lot of indexes, many of which are very heavy on duplicates.

The implementation of Postgres’ MVCC makes the change even more important than it seems on the surface. Rows being deleted or changed will have duplicated index values pointing to different versions of the same logical record, and even when those duplicates are no longer needed, the system would generally have to wait for a vacuum to reclaim them, before which there was ample opportunity for bloat. Databases where updates lead to many simultaneously living versions of the same row could also see major benefits from this change.

#### Why it’s important (#important)

Postgres users with large installations will already know this, but one should underestimate the importance of index size. They start small, but can get very big indeed.

For the last six months I’ve been dealing with a problem at work involving data size and retention, and in this one particular Mongo collection we have _just indexes_ that range in many 10s of TBs. These large indexes are an extension of the underlying data being indexed being large, but it's not as much as you might think -- less than one order of magnitude larger.

Its indexes are now so large that they now drive product decisions -- we’ll introduce hacks of considerable size or skip proper fixes to avoid having to raise a new index, which takes on the order of weeks to create, and is a large figure with a lot of zeros on the end every month in infrastructure costs to maintain.
...
A very common development scenario was one where you wanted to drop a database (most often to immediately re-create it), but were stopped in your tracks when Postgres wouldn’t let that happen even if one other client was connected. This would lead you down the rabbit hole of figuring out which terminal tab or running program was still holding an open connection so that you could run it down. Now imagine that happening twenty times a week. It's theoretically a useful safety feature, but in practice ate a lot of precious time.
...
The nuance around how IP addresses and ranges sort relative to each other, and supporting all the rules around IPv4 as well as IPv6 means that this change is a little more difficult than it might appear. It sure stretched my remedial skills in C and bit shifting close to their breaking point. I wrote a [detailed article](/sortsupport-inet) on how it all works.
...
A scene from the _Islands_ episode of _II_ where a hatching group of marine iguanas on Fernandina Island have to make their way passed a wall of Galapagos racer snakes to their new life on the rocky coastlines has stuck with me to this day. You see the entirety of the journey of specific iguanas -- the unlucky of which are ravaged in scenes of brutal natural violence -- and others as they blow by their would-be predators and escape to the safety of the sea. Like, how is it even possible to get those shots? Let alone in crystal clear detail and cinemagraphic perfection down to the smallest detail. It blows my mind.

But if there's one criticism to be levied against _Planet Earth_, it's that its producers are obviously going to incredible lengths to gloss over the human impact on these natural ecosystems. Humanity is rarely even mentioned, let alone shown on-screen. This is done as a courtesy to the viewer, who generally has enough to worry about in their life, and doesn't want to spend their evenings watching doom and gloom, but seeing blue whales, snow leopards, and river dolphins in apparently pristine natural environments has the effect of convincing us that there's plenty of natural world left, and that maybe we're not so bad after all.
