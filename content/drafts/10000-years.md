...
article was published, and the latest optimistically
scheduled to fire 10,000 years from now. Here's the first
in the sequence:
...
edges over the years, and it stands to reason that the
company will eventually undertake a project to revitalize
it, and that will likely spell the end of the current API
...
these are largely applicable to the software industry. I
might reword them like so:
